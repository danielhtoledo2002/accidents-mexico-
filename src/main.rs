...
async fn give_type_accident(tyype: &str, anio : i32, state : &str, pool: &sqlx::Pool<MySql>) -> Resultado5 {
...
async fn give_day_hour_accidents(pool: &sqlx::Pool<MySql>) -> Resultado4{
        ...
        let mut y: Vec<(String, i32, i32)> = make_query::<(String, i32, i32)>("Select diasemana, id_hora, count(diasemana) as num_accidentes
...
async fn give_total_accidents_hours(pool: &sqlx::Pool<MySql>) -> Resultado2{
    ...
    let mut y :Vec<(i32, i32)> = make_query::<(i32, i32)>("select id_hora, count(id) from atus_anual group by id_hora;", pool).await.unwrap();
    Resultado2{
        x: y.iter().map(|a| a.0).collect(),
        y : y.iter().map(|a| a.1).collect()
    }
...
async fn give_types_accidents(pool: &sqlx::Pool<MySql>) -> Resultado{
        let mut y :Vec<(String, i32)> = make_query::<(String, i32)>("select tipaccid, count(id)as total from atus_anual group by tipaccid order by total desc ;
", pool).await.unwrap();
    Resultado{
        x: y.iter().map(|a| a.0.clone()).collect(),
        y : y.iter().map(|a| a.1).collect()
...
async fn give_man_woman_accidents(pool: &sqlx::Pool<MySql>) -> Resultado{
        let mut y: Vec<(String, i32)> = make_query::<(String, i32)>("Select sexo, count(sexo) as num_accidentes
        From atus_anual where sexo ='Hombre' or sexo= 'Mujer' Group by sexo;", pool).await.unwrap();
...
async fn give_day_number_accidents(pool: &sqlx::Pool<MySql>) -> Resultado{
        let mut y : Vec<(String, i32)> = make_query::<(String, i32)>("Select diasemana, count(diasemana) as num_accidentes
        from atus_anual where diasemana != 'Certificado cero'
        Group by diasemana;", pool).await.unwrap();
...
async fn give_age_accidents(pool: &sqlx::Pool<MySql>) -> Resultado2{
        let mut y : Vec<(i32, i32)> = make_query::<(i32, i32)>("select id_edad, count(id_edad) as total from atus_anual group by id_edad order by total desc;", pool).await.unwrap();
        Resultado2{
            x : y.iter().map(|a| a.0).collect(),
            y : y.iter().map(|a| a.1).collect(),
        }
...
async fn give_aliento_noaliento(pool: &sqlx::Pool<MySql>) -> Resultado{
        let mut y : Vec<(String, i32)> = make_query::<(String, i32)>("select aliento, count(aliento) from atus_anual group by aliento;", pool).await.unwrap();
        Resultado{
            x : y.iter().map(|a| a.0.clone()).collect(),
            y : y.iter().map(|a| a.1).collect(),
        }
...
sum(OTROVEHIC) as otro From  atus_anual;", pool).await.unwrap();
        Resultado9{
...
async fn give_cause_accident(pool: &sqlx::Pool<MySql>) -> Resultado{
        let mut y : Vec<(String, i32)> = make_query::<(String, i32)>("select causaacci, count(causaacci) from atus_anual group by causaacci;", pool).await.unwrap();
        Resultado{
            x : y.iter().map(|a| a.0.clone()).collect(),
            y : y.iter().map(|a| a.1).collect(),
        }
